...
pull_linux(){
	...
	echo "patching..."
        patch -t -d linux -p1 < ${temp_root_dir}/1.txt
...
pull_all(){
	...
	pull_toolchain
	pull_buildroot
	cp -f ${temp_root_dir}/licheepi_nano_usb_defconfig ${temp_root_dir}/${linux_dir}/arch/arm/configs/licheepi_nano_usb_defconfig
...
upload(){
    ...
    ./ghr_v0.13.0_linux_amd64/ghr --debug -u xjx00 -r licheepi-autobuild -b "$GIT_RELTEXT" $GIT_TAG ${temp_root_dir}/output/
}
#clean output dir=========================================================
clean_output_dir(){
...
if [ "${1}" = "nano_linux" ]; then
	linux_config_file="licheepi_nano_usb_defconfig"
	u_boot_config_file="licheepi_nano_defconfig"
	u_boot_boot_cmd_file="tf_boot.cmd"
	build_only_linux
	upload
